...
\setmintedinline{bgcolor={Beige!50!white}}

\usepackage{mdframed}
\mdfsetup{backgroundcolor=Beige!50!white,hidealllines=true}
...
If you're writing an undergraduate thesis. \marginnote{Remember to specify
             your \mintinline{LaTeX}{\faculty} and \mintinline{LaTeX}{\bachprogramme} if using this option.} There will then
...
Modify the following lines in the template to suit your own thesis. These information will be used to generate the cover and title pages, as well as various forms in the thesis. Don't forget to specify your \mintinline{LaTeX}{\faculty} and \mintinline{LaTeX}{\bachprogramme} if you have specified the \texttt{undergrad} document option.
...
The Supervisory Commitee approval is created in a similar manner, using the \mintinline{LaTeX}{supervisoryapproval} environment:
...
I highly recommend that each chapter be written in a separate file. For example, \directory{sample-chap-intro.tex} may have the contents
...
\end{minted}
 ...
 \marginnote{The \mintinline{bash}{!TEX ROOT} directive
...
Graphics files, figures, tables, itemize and enumerated lists, etc are created using the standard \LaTeX{} commands. See \directory{sample-chap-intro.tex} for some examples, including how to create subfigures (and by extension, subtables).

\subsection{Citatons and Bibliography}

Citations and bibliography are done using Bib\TeX{}, adopting the \smallcaps{APA} style. Both \texttt{natbib} and \texttt{apacite} commands can be used -- in most cases you'll just need to remember the following:
...
\mintinline{LaTeX}{\citep[p.5]} & (John, 2004, p.5)\\
